    ...
    execute: function () {
      ...
      _export("ThunderRunning", ThunderRunning = (_dec = ccclass('ThunderRunning'), _dec(_class = class ThunderRunning extends (_crd && Weapon === void 0 ? (_reportPossibleCrUseOfWeapon({
        ...
        init(flyDir) {
          ...
          this.initSkillData();
          var p = this.node.getWorldPosition(); // let dir = em.dispatch("getHeroControlProperty", "_curAimDir");
        ...
        weaponMove(deltaTime) {
          ...
          if (this._flyDir.y == 0) {
            var x = deltaTime * this._moveSpeed;
            var y = this.getPathLikeSin(wp.x + x - this._startP.x);
            this._curMoveDistance += x;
            this.node.setWorldPosition(wp.x + x * this._flyDir.x, this._startP.y + y, 0);
          } else {
            var _y = deltaTime * this._moveSpeed;

            var _x = this.getPathLikeSin(wp.y + _y - this._startP.y);

            this._curMoveDistance += _y;
            this.node.setWorldPosition(this._startP.x + _x, wp.y + _y * this._flyDir.y, 0);
          }
        }
...
//# sourceMappingURL=a4668e0e332a0f9852aeb81c435853e030b2c7ea.js.map