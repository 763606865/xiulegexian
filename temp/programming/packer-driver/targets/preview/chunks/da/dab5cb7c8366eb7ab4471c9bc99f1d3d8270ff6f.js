  ...
  return {
    ...
    execute: function () {
      ...
      _export("StorageSpace", StorageSpace = (_dec = ccclass('StorageSpace'), _dec2 = property(JsonAsset), _dec(_class = (_class2 = class StorageSpace extends Component {
        ...
        initItemData() {
          var all = this.itemDataJson.json;
          all.forEach(element => {
            var id = element.id;
          ...
          if (null !== data) {
            for (var key in data) {
              if (Object.prototype.hasOwnProperty.call(data, key)) {
                var element = data[key]; // this.addItemToSS(key, element, false);
        ...
        addItemToSS(id_name, total) {
          ...
          var id = this._itemData[id_name].id;
        ...
        reduceItemFromSS(id_name, total) {
          ...
          var result = this._IM.reduceItem(id, total);
        ...
        getAllPills() {
          ...
          for (var id in this._IM._itemList) {
        ...
        getItemsRewardByAds() {
          ...
          var t1 = 1;
          var t2 = 5 + Math.random() * 5 | 0; //最少5个 最多10个妖丹

          if (!data || data.curLv < 5) {
            this.addItemToSS("一阶宝箱", t1);
            this.addItemToSS("炼气丹", t2);
            var tips = "获得物品：一阶宝箱x" + t1 + "、炼气丹x" + t2;
            (_crd && em === void 0 ? (_reportPossibleCrUseOfem({
              error: Error()
            ...
            var _tips = "获得物品：二阶宝箱x" + t1 + "筑基丹x" + t2;

            (_crd && em === void 0 ? (_reportPossibleCrUseOfem({
              error: Error()
            }), em) : em).dispatch("tipsViewShow", _tips);
          } else if (data.curLv < 31) {
            this.addItemToSS("三阶宝箱", t1);
            this.addItemToSS("金元丹", t2);

            var _tips2 = "获得物品：三阶宝箱x" + t1 + "金元丹x" + t2;

            (_crd && em === void 0 ? (_reportPossibleCrUseOfem({
              error: Error()
            }), em) : em).dispatch("tipsViewShow", _tips2);
          } else if (data.curLv < 47) {
            this.addItemToSS("四阶宝箱", t1);
            this.addItemToSS("元婴丹", t2);

            var _tips3 = "获得物品：四阶宝箱x" + t1 + "元婴丹x" + t2;

            (_crd && em === void 0 ? (_reportPossibleCrUseOfem({
              error: Error()
            }), em) : em).dispatch("tipsViewShow", _tips3);
          } else {
            this.addItemToSS("五阶宝箱", t1);
            this.addItemToSS("化神丹", t2);

            var _tips4 = "获得物品：五阶宝箱x" + t1 + "化神丹x" + t2;

            (_crd && em === void 0 ? (_reportPossibleCrUseOfem({
              error: Error()
            }), em) : em).dispatch("tipsViewShow", _tips4);
          }

          ;
          var ssNode = find("Canvas/menuLayer/HeroInfoLayer/storageSpace");

          if (ssNode.active) {
            var script = find("Canvas/menuLayer/HeroInfoLayer").getComponent("HeroInfoLayer");
...
//# sourceMappingURL=dab5cb7c8366eb7ab4471c9bc99f1d3d8270ff6f.js.map