  ...
  return {
    ...
    execute: function () {
      ...
      _export("SkillBookLayer", SkillBookLayer = (_dec = ccclass('SkillBookLayer'), _dec2 = property([SpriteFrame]), _dec3 = property(Node), _dec4 = property([SpriteFrame]), _dec(_class = (_class2 = class SkillBookLayer extends Component {
        ...
        onLoad() {
          var prefab = find("switchBook/bookPrefab", this.node);
        ...
        dynamicLoadPrefabs() {
          var dir1 = "prefabs/mainMenu/skillBookClass";
          var dir2 = "prefabs/mainMenu/skillBook";
        ...
        initBookListView() {
          ...
          var list = this.getSkillBookClassList();

          var _loop = function _loop() {
            if (Object.prototype.hasOwnProperty.call(list, key)) {
              var arr = list[key];
              var bookClass = instantiate(_this._skillBookClass);
              bookClass.parent = _this._bookContent; // bookClass.getChildByName("Label").getComponent(Label).string = key;

              switch (key) {
                case "绝技":
                  bookClass.getChildByName("type").getComponent(Sprite).spriteFrame = _this.typeSFs[0];
                  break;

                case "秘法":
                  bookClass.getChildByName("type").getComponent(Sprite).spriteFrame = _this.typeSFs[1];
                  break;

                case "身法":
                  bookClass.getChildByName("type").getComponent(Sprite).spriteFrame = _this.typeSFs[2];
                  break;

                case "神通":
                  bookClass.getChildByName("type").getComponent(Sprite).spriteFrame = _this.typeSFs[3];
                  break;

                case "武技":
                  bookClass.getChildByName("type").getComponent(Sprite).spriteFrame = _this.typeSFs[4];
                  break;

                case "心法":
                  bookClass.getChildByName("type").getComponent(Sprite).spriteFrame = _this.typeSFs[5];
                  break;
              ...
              arr.forEach(data => {
                var book = instantiate(_this._skillBook);
                var loadUrl = "images/icons/icon_" + data.name2 + "/spriteFrame";
                (_crd && em === void 0 ? (_reportPossibleCrUseOfem({
                  error: Error()
                }), em) : em).dispatch("loadTheDirResources", loadUrl, assets => book.getChildByName("Sprite").getComponent(Sprite).spriteFrame = assets);

                if (_this._finishBookList.indexOf(data.name) < 0) {
                  //未学习的功法
                  book.getComponent(Sprite).spriteFrame = _this.skillBg[0];
                  ...
                  if ((_crd && em === void 0 ? (_reportPossibleCrUseOfem({
                    ...
                    var anim = book.getComponent(Animation);
                  ...
                  book.getComponent(Sprite).spriteFrame = _this.skillBg[1]; // book.getChildByName("Label").getComponent(Label).color = new Color(255, 204, 0, 255);
                ...
                (_crd && glf === void 0 ? (_reportPossibleCrUseOfglf({
                  error: Error()
                }), glf) : glf).createButton(_this.node, book, "SkillBookLayer", "onBtnShowBookDetail", data.name);
                _this._bookPrefabList[data.name] = book;
              }); // let total = Math.ceil(arr.length / 4);//4为适配结果，具体根据效果展示

              var total = Math.ceil(arr.length / 5); //5为适配结果，具体根据效果展示
              ...
              var height = 280 * total + find("/type", bookClass).getComponent(UITransform).contentSize.y; //120 skillBook 预制件的高度
          ...
          if (!(_crd && em === void 0 ? (_reportPossibleCrUseOfem({
            ...
            var book = this._bookPrefabList[this._guideRewardBookName];
        ...
        updateBookListView() {
          for (var key in this._bookPrefabList) {
            if (Object.prototype.hasOwnProperty.call(this._bookPrefabList, key)) {
              var book = this._bookPrefabList[key];
        ...
        isCompleteLoading() {
          for (var key in this._loadingList) {
            if (Object.prototype.hasOwnProperty.call(this._loadingList, key)) {
              var isEnd = this._loadingList[key];
        ...
        onBtnShowBookDetail(e, p) {
          var node = find("bookDetail", this.node);
          var label = find("/bookDetail/study/Label", this.node).getComponent(Label);
          label.string = "领悟";
          node.active = true;
        ...
        onBtnStudyBook() {
          ...
          var flag = (_crd && em === void 0 ? (_reportPossibleCrUseOfem({
            error: Error()
          ...
          if (flag) {
            ...
            var book = this._bookPrefabList[this._curSelectBook];
            var anim = book.getComponent(Animation);
            anim.stop();
        ...
        initUsingBook() {
          ...
          var _loop2 = function _loop2() {
            if (Object.prototype.hasOwnProperty.call(_this2._usingBookList, type)) {
              var skillName = _this2._usingBookList[type];

              if ("" !== skillName) {
                var sprite = find("usingBook/" + type, _this2.node).getChildByName("sprite").getComponent(Sprite);
          ...
          for (var type in this._usingBookList) {
            _loop2();
          }
        } //点击切换携带功法按钮
        ...
        initSwitchBook(type) {
          var par = find("switchBook/bg/content", this.node);
          this._finishBookList;
        ...
        switchBook(e, p) {
          ...
          var type = ps[0];
          var skillName = ps[1];
          var sprite = find("usingBook/" + type, this.node).getChildByName("sprite").getComponent(Sprite);
          var loadUrl = "images/icons/icon_" + skillName + "/spriteFrame";
          (_crd && em === void 0 ? (_reportPossibleCrUseOfem({
            error: Error()
          }), em) : em).dispatch("loadTheDirResources", loadUrl, assets => sprite.spriteFrame = assets);
        ...
        startSkillBookLayerGuide() {
          var guideTips = "首次进入，随机获取一本一阶功法（剑雨术、一剑隔世和万剑归冢）。每次进入功法界面，如果看到不断闪烁的功法，则表明此功法可以掌握，请赶快领悟并使用。";
          (_crd && em === void 0 ? (_reportPossibleCrUseOfem({
            error: Error()
        ...
        startGuideSetSkill() {
          var par = find("Canvas/menuLayer/SkillBookLayer/usingBook/" + this._guideRewardBookType);
          Tween.stopAllByTag(4);
          var guide = this.guideFinger;
          guide.active = true;
          guide.parent = par;
...
//# sourceMappingURL=34ebded269dfbe8fe25364b069dd8ccb887ae80d.js.map