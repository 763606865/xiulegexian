    ...
    execute: function () {
      ...
      _export("StageManager", StageManager = (_dec = ccclass('StageManager'), _dec2 = property(JsonAsset), _dec3 = property(JsonAsset), _dec4 = property(JsonAsset), _dec5 = property(JsonAsset), _dec6 = property([Prefab]), _dec(_class = (_class2 = class StageManager extends Component {
        ...
        startManyMonsterTimer() {
          var max = this._curData.max > (_crd && monsterData === void 0 ? (_reportPossibleCrUseOfmonsterData({
            error: Error()
          ...
          if (this._monsterArmyConfig) {
            var index = this._monsterArmyConfig.time.indexOf(this._curStageTime);

            if (index > -1) {
              // if (config.queue[index] >= 11001) em.dispatch("playAnimMassMonsterComing");
              var key = (_crd && monsterData === void 0 ? (_reportPossibleCrUseOfmonsterData({
                error: Error()
              }), monsterData) : monsterData).queueMappingList[this._monsterArmyConfig.queue[index]];
              var queue = (_crd && monsterData === void 0 ? (_reportPossibleCrUseOfmonsterData({
                error: Error()
              ...
              var monsterID = this._curData.id[Math.random() * this._curData.id.length | 0];
          ...
          if (this._curStageTime > this._curData.time) {
            ...
            if (curTotal < max) {
              ...
              if (this._createCount > this._bossConfig.startCount && this._createCount % this._bossConfig.mod == 0 && this._curLeaderTotal < this._bossConfig.maxTotal) {
                ...
                var type = this._bossConfig.type[Math.random() * this._bossConfig.type.length | 0];
        ...
        distributeReward(string, isPass) {
          if (isPass === void 0) {
            isPass = false;
          }
        ...
        startDistributeReward() {
          var string = this._rewardData.string;
          var isPass = this._rewardData.isPass;
          var isDouble = this._rewardData.isDouble;
          var passReward = this.getPassRewardContent(isPass);

          if (isDouble) {
            for (var reward of passReward) {
              reward.total *= 2;
            }
        ...
        createMonsterByCurData() {
          var index = this.getIndexByRatio(this._curData.ratio);
          var key = (_crd && monsterData === void 0 ? (_reportPossibleCrUseOfmonsterData({
            error: Error()
          ...
          var dir = (_crd && monsterData === void 0 ? (_reportPossibleCrUseOfmonsterData({
            error: Error()
        ...
        createBoss() {
          var prefab = this.bossPrefabs[this._bossConfig.bossId - 1];

          if (prefab) {
            var boss = instantiate(prefab); // boss.parent = find("Canvas/enemyLayer");
        ...
        removeBoss() {
          var boss = find("Canvas/bossLayer").children[0];
        ...
        updateTimeLabel() {
          ...
          var time = " 时 间 ： " + this._curStageTime + " ";
          var progress = "（进度" + Math.floor(this.getCurStageProgress() * 100) + "%）";
        ...
        getPassRewardContent(isPass) {
          var config = this._rewardConfig[this._curStageName];
          var rewardArr = []; //击杀奖励

          config.monsterReward.forEach((id, index) => {
            var ratio = config.monsterRewardRatio[index];
            var total = 0;

            for (var i = 0; i < this._killCount; i++) {
          ...
          config.leaderReward.forEach((id, index) => {
            var ratio = config.leaderRewardRatio[index];
            var total = 0;

            for (var i = 0; i < this._killLeaderTotal; i++) {
              var random = Math.random();
              if (random <= ratio) total++;
            }
          ...
          if (isPass) {
            config.passReward.forEach((id, index) => {
              var ratio = config.passRewardRatio[index];
        ...
        getCurStageProgress() {
          var progress = this._curStageTime / this._curStageMaxTime;
...
//# sourceMappingURL=d426f3c93615e20744622628f630422574cf843e.js.map