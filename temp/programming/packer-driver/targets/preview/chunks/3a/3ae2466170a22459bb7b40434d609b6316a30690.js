  ...
  return {
    ...
    execute: function () {
      ...
      _export("LevelManager", LevelManager = class LevelManager {
        ...
        upgrade() {
          var len = this.maxLevel;
          var start = this.level + 1;
          var afterLv;

          for (var i = start; i < len; i++) {
            // let exp = this.levelMappingExpList[i].exp;
            var exp = this.levelMappingExpList[i - 1].exp;
        ...
        getExpProgress() {
          if (this.level == 1) return this.curExp / this.levelMappingExpList[this.level].exp;else {
            var fontLvExp = this.levelMappingExpList[this.level - 1].exp;
            var curLvExp = this.levelMappingExpList[this.level - 0].exp; // console.log("fontLvExp",fontLvExp);
...
//# sourceMappingURL=3ae2466170a22459bb7b40434d609b6316a30690.js.map