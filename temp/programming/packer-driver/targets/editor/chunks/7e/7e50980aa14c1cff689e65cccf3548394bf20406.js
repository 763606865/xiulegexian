  ...
  return {
    ...
    execute: function () {
      ...
      _export("HeroControl", HeroControl = (_dec = ccclass('HeroControl'), _dec2 = property(Sprite), _dec3 = property(Sprite), _dec4 = property(Node), _dec5 = property(Sprite), _dec6 = property(JsonAsset), _dec7 = property(JsonAsset), _dec8 = property(Label), _dec9 = property(Node), _dec10 = property(Prefab), _dec11 = property(Node), _dec12 = property(Prefab), _dec13 = property([Material]), _dec(_class = (_class2 = class HeroControl extends Component {
        ...
        initHeroDataByLv() {
          let a = (_crd && em === void 0 ? (_reportPossibleCrUseOfem({
            error: Error()
        ...
        isCollideObs(x, y) {
          let obsArr = (_crd && em === void 0 ? (_reportPossibleCrUseOfem({
            error: Error()
          }), em) : em).dispatch("usingMapLayerFun", "getAllObs");
          let curPos = this.heroSprite.getWorldPosition();
          let collider = this.heroSprite.getComponent(Collider2D); // return false;
        ...
        updateBloodProgress(changeValue) {
          ...
          if (changeValue < 0) {
            //受到伤害
            let damageReduce = (_crd && em === void 0 ? (_reportPossibleCrUseOfem({
              error: Error()
        ...
        */
        ...
        seekTreeFirstEnemyDir() {
          let tree = (_crd && em === void 0 ? (_reportPossibleCrUseOfem({
            error: Error()
          }), em) : em).dispatch("getCurMonsterQuadtree");
          let rect = this.heroSprite.getComponent(BoxCollider2D).worldAABB;
          let res = tree.retrieve(rect); // console.log("seekTreeFirstEnemyDir",res);
          //随机返回一个附近位置

          if (res.length) {
            let rect = res[0]; //四叉树位置锁定修正
        ...
        */
        ...
        seekTreeRandomNEnemyDir(n) {
          ...
          let rect = this.heroSprite.getComponent(BoxCollider2D).worldAABB;
          let res = tree.retrieve(rect); // console.log("seekTreeRandomNEnemyDir",res);
          ...
          if (res.length) {
            ...
            for (let i = 0; i < n; i++) {
              ...
              let dir = (_crd && glf === void 0 ? (_reportPossibleCrUseOfglf({
                error: Error()
              }), glf) : glf).getTwoPointFlyDir(targetPos, wp);
              arr.push(dir);
            }
        ...
        */
        ...
        seekTreeRandomOneEnemyPos() {
          ...
          let rect = this.heroSprite.getComponent(BoxCollider2D).worldAABB;
          let res = tree.retrieve(rect); //随机返回一个附近位置

          if (res.length) {
            let rect = res[Math.random() * res.length | 0]; //四叉树位置锁定修正
        ...
        */
        ...
        getCurMoveSpeed() {
          ...
          let msSV = (_crd && em === void 0 ? (_reportPossibleCrUseOfem({
            error: Error()
        ...
        getCurDamage() {
          ...
          let d2 = (_crd && em === void 0 ? (_reportPossibleCrUseOfem({
            error: Error()
          }), em) : em).dispatch("usingSkillManagerFun", "getDamageValueFromSecretSkill");
          let dp = (_crd && em === void 0 ? (_reportPossibleCrUseOfem({
            error: Error()
          }), em) : em).dispatch("usingSkillManagerFun", "getPercentageDamageFromSecretSkill"); //秘法提供的加成

          let dTotal = (d1 + d2) * (1 + this._bonusPercentageDamage + this._percentageDamage + dp + this._tempPercentageDamage); // console.log("d1", d1);
          // console.log("d2", d2);
          // console.log("dp", dp);
        ...
        */
        ...
        usingSkillFlyingThunderGod() {
          if (this._flyingThunderGodCount % 2 == 0) {
            ...
            prefab.parent = find("Canvas/bulletLayer");
            let wp = this.getHeroWorldPos(); // prefab.setWorldPosition(wp.x,wp.y,wp.z);
...
//# sourceMappingURL=7e50980aa14c1cff689e65cccf3548394bf20406.js.map