    ...
    execute: function () {
      ...
      _export("FlySword", FlySword = (_dec = ccclass('FlySword'), _dec(_class = class FlySword extends (_crd && Weapon === void 0 ? (_reportPossibleCrUseOfWeapon({
        ...
        init(data, lv) {
          ...
          let weaponName = data.name; // this.initBoxCollider(tagData.sword);
        ...
        flyToTp(dt) {
          ...
          let cp = this.node.getWorldPosition(); //四叉树位置锁定修正

          let x = this._curTargetRect.x + this._curTargetRect.width / 2 - cp.x;
          let y = this._curTargetRect.y + this._curTargetRect.height / 2 - cp.y; // let x = this._curTargetRect.x - cp.x;
          // let y = this._curTargetRect.y - cp.y;

          let flyDis = dt * this._moveSpeed; //可以到达目标点
        ...
        flyToHero(dt) {
          ...
          let x = hwp.x - cp.x;
          let y = hwp.y - cp.y; // let flyDis = dt * this._moveSpeed;
...
//# sourceMappingURL=bb3612e4411304ee8980b9e393b0dc87fa385f4f.js.map